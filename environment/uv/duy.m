function [echogram] = duy(echogram, mldpath_weekly, tagyear)
% Scripts that reads and extracts the duy data



% Load duy files
duyfiles_weekly = dir(mldpath_weekly);

% List of dates for duy extraction
...
%% Extract daily duy at the corresponding cruise lon/lat 
lon_duy = [];
lat_duy = [];
time_duy = [];
distance_duy = [];
...
% Extract weekly duy at the corresponding cruise lon/lat 
...
for k = 1:size(YMD_unique,1)
	...
	for l = 1 : length(duyfiles_weekly)		
		if strfind(duyfiles_weekly(l).name,tagyear)
		if strfind(duyfiles_weekly(l).name(26:35),match_name)
			[weekly_duy, lon_duy, lat_duy, time_duy, distance_duy] = weekly_duy_extract(echogram, [mldpath_weekly,duyfiles_weekly(l).name], str2num(D_unique(k,:)), weekly_duy, lon_duy, lat_duy, time_duy, distance_duy);
			flag = 1;
%             end
		end
		end
...
echogram.duy.daily = weekly_duy;
echogram.duy.weekly = weekly_duy;
echogram.duy.lon = lon_duy;
echogram.duy.lat = lat_duy;
echogram.duy.time = time_duy;
echogram.duy.dist = distance_duy;
