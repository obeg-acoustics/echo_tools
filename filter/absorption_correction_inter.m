function [echogram] = absorption_correction_inter(echogram)
...
for k = 1:length(echogram.pings)
    ...
    [Tint,Sint] = getenvironment_inter(depths,echogram.pings(k).lon,echogram.pings(k).lat,times(:,1),times(:,2),times(:,3));
    ...
    [Tint,Sint] = getenvironment_inter(depthsbar,echogram.pings(k).lon,echogram.pings(k).lat,times(:,1),times(:,2),times(:,3));
