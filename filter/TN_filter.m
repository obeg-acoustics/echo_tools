...
for k=1:length(echogram.pings) % Loop upon all the frequencies
	...
	% SMOOTHED TRANSECT
        ...
        if (size(SvNDprcblock,2)-c*chunksize)>TNn
            SvNDprcblock(:,c*chunksize+1:end) = colfilt(SvND(:,c*chunksize+1:end),[TNm TNn],'sliding',fcn_prctile);
        else
            SvNDprcblock(:,c*chunksize+1:end) = colfilt(SvND(:,c*chunksize+1:end),[TNm floor(((size(SvNDprcblock,2)-c*chunksize))/2)],'sliding',fcn_prctile); 
        end
