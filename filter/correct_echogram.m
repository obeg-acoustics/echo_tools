...
for k = 1:length(echogram.pings)
    ...
    % Corrects anomalous time stamps
    if isfield(echogram.pings(k),'time')
        tmax = echogram.pings(k).time(end);
        ind  = find(echogram.pings(k).time>tmax);
        if ~isempty(ind)
            echogram.pings(k).time(ind) = (echogram.pings(k).time(ind-1)+echogram.pings(k).time(ind+1))/2;
        end
    end
end
