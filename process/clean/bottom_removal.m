function [echogram] = bottom_removal(echogram, meters_above_bottom_removal)
...
for i=1:length(echogram.pings)
	for j=1:length(ind)
		echogram.mask(i).SvBot(echogram.bottom.bottom_indexes(ind(j))-indexes_above_bottom_removal:end,ind(j)) = NaN;
	end
end
...
for i=1:length(echogram.pings)
	for j=1:length(ind)
		echogram.mask(i).SvBot(1:indexes_upper_layer_removal,:) = NaN;
	end
end
