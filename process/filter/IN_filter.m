...
%	Ldelta = echogram.pings(i).Sv(:,INn+1:end-INn) - echogram.pings(i).Sv(:,1:end-2*INn); % We calculate the difference in time between the column of index i, and the column of index i-INn
	Lind = find(echogram.pings(i).Sv(:,INn+1:end-INn) - echogram.pings(i).Sv(:,1:end-2*INn) > INthreshold); % We find in the new matrix the indexes where the Sv value is above INthreshold, the threshold settled before.
...
%	Rdelta = echogram.pings(i).Sv(:,INn+1:end-INn) - echogram.pings(i).Sv(:,2*INn+1:end); % We calculate the difference in time between the column of index i, and the column of index i+INn
	Rind = find(echogram.pings(i).Sv(:,INn+1:end-INn) - echogram.pings(i).Sv(:,2*INn+1:end) > INthreshold); % We find in the new matrix the indexes where the Sv value is above INthreshold, the threshold settled before.
